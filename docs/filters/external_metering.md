...
| `provider` | string | no | Static provider name attributed to emitted `CloudEvents`. On the ext-proc data path Envoy performs routing, so the praxis router never runs and `ctx.cluster_name()` is always `None`; this static value is the only way to attribute usage to a provider there. When unset, the filter falls back to the routed cluster name (populated on the in-process data path) and finally to an empty string. |
